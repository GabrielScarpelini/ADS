...
# lembre-se que é no raw tipo Json
...
@app.route('/professores/delete/<int:id_teacher>')
def deleteTeacher(id_teacher):
    for prof in database['PROFESSOR']:
        if prof['id'] == id_teacher:
            database['PROFESSOR'].remove(prof)
            return jsonify(database['PROFESSOR'])
    return f'professor não existe'
